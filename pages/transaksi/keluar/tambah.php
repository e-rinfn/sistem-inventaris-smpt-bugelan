...
$barang = $pdo->query("SELECT id_barang, kode_barang, nama_barang, stok FROM barang WHERE stok > 0 ORDER BY nama_barang")->fetchAll();
...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    try {
        ...
        $stmt = $pdo->prepare("INSERT INTO barang_keluar (
            id_barang, id_pengguna, jumlah, tanggal_keluar, 
            penerima, keperluan, keterangan
        ) VALUES (?, ?, ?, ?, ?, ?, ?)");
        ...
        header("Location: index.php?success=Transaksi+barang+keluar+berhasil+disimpan");
        exit();
    } catch (Exception $e) {
        $pdo->rollBack();
...
<body>
    ...
    <div class="layout-wrapper layout-content-navbar">
        ...
        <!-- Core JS -->
        <script>
            // Validasi stok saat memilih barang dan mengisi jumlah
